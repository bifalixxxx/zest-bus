// api/delays.js — Retards par ligne via TomTom Routing API (trajet complet, trafic vs no-traffic)
// Env: TOMTOM_KEY
// Cache mémoire 60s (≤ 1 appel/min même avec beaucoup d’usagers)

const KEY = process.env.TOMTOM_KEY;
const AMPLIFY = 2;            // multiplier la perception (2 = x2)
const MIN_IF_CONGESTED = 5;   // minimum si congestion détectée
const MAX_MIN = 60;           // plafond
const CACHE_TTL = 60 * 1000;  // 60s
let cache = { ts: 0, data: null };

// Itinéraires représentatifs (start, via..., end)
const LINE_ROUTES = {
...
export default async function handler(req,res){
  try{
    ...
    await Promise.all(Object.keys(LINE_ROUTES).map(async ln=>{
      try{ out[ln] = await fetchDelayForRoute(LINE_ROUTES[ln]); }
      catch{ out[ln] = 0; }
    }));
